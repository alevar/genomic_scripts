...
struct Globals{
    std::ofstream out_grouper_fp;
    std::ofstream out_grouper_single_fp;
    bool consider_strand;
} globals;
...
int single_intersection(std::array<int,3>& i1,std::array<int,3>& i2,std::array<int,3>& res){ // returns number of bases of overlap
...
//                }
    ...
    int get_elen(){
        return chain_len(this->exons);
    }

    int length(){
        return this->get_elen();
    }

    int get_total_overlap(TX& tx){
        std::array<int,3> i1{this->get_start(),this->get_end(),0};
        std::array<int,3> i2{tx.get_start(),tx.get_end(),0};
        std::array<int,3> int_res;
        return single_intersection(i1,i2,int_res);
    }
...
public:
    ...
    int split(std::vector<Locus>& res){ // split locus if any transcripts within are disconnected (non-overlaping) - put new loci into the vector
...
public:
    ...
    void sort(bool consider_strand){
        if(consider_strand){ // use default
...
public:
    Bundle()=default;
    Bundle(bool consider_strand){
        this->consider_strand=consider_strand;
    };
    ~Bundle()= default;
    ...
    bool can_add(TX& t){
        if(this->size==0){
            ...
            if(std::strcmp(this->seqid.c_str(),t.get_seqid().c_str())==0 && (!this->consider_strand || this->strand==t.get_strand())){
    ...
    int process(Globals& globals_local){ // returns true if new gene_id has been assigned
        ...
        int q1_idx=0;
        for(auto& txv : compat){
            TX& q1 = this->txs[q1_idx];

            if(txv.size()==1){
                globals_local.out_grouper_single_fp<<this->bid<<"\t"<<
                                                   this->seqid+":"+std::to_string(this->start)+"-"+std::to_string(this->end)+"\t"<<
                                                   q1.get_tid()<<"\t"<<
                                                   q1.get_gid()<<"\t"<<
                                                   q1.get_strand()<<"\t"<<std::endl;
                continue;
            }

            int q2_idx=0;
            for(auto& val : txv){
                // skip duplicates and self-to-self
                if(q2_idx<=q1_idx){
                    q2_idx+=1;
                    continue;
                }
                TX& q2 = this->txs[q2_idx];
                // output as a separate column whether strand matches - that way we still get full range of statuses (even though most are likely useless)
                globals_local.out_grouper_fp<<this->bid<<"\t"<<
                                              this->seqid+":"+std::to_string(this->start)+"-"+std::to_string(this->end)+"\t"<<
                                              q1.get_tid()<<"\t"<<
                                              q2.get_tid()<<"\t"<<
                                              q1.get_attribute("gene_id")<<"\t"<<
                                              q2.get_attribute("gene_id")<<"\t"<<
                                              q1.get_strand()<<"\t"<<
                                              q2.get_strand()<<"\t"<<
                                              val.first<<"\t"<<
                                              std::to_string(q1.num_introns_shared(q2))<<"\t"<<
                                              std::to_string(q1.intron_count())<<"\t"<<
                                              std::to_string(q1.intron_count())<<"\t"<<
                                              std::to_string(q1.get_total_overlap(q2))<<"\t"<<
                                              val.second<<"\t"<<
                                              q1.get_elen()<<"\t"<<
                                              q2.get_elen()<<"\t"<<std::endl;
                q2_idx+=1;
            }
            q1_idx+=1;
        }
...
int run(const std::string& novel_gtf_fname,const std::string& out_fname){
    globals.out_grouper_fp.open(out_fname);
    globals.out_grouper_fp<<"bid\t"<<
                            "bid_coords\t"<<
                            "tid1\t"<<
                            "tid2\t"<<
                            "gid1\t"<<
    ...
    globals.out_grouper_single_fp.open(out_fname+".single");
    globals.out_grouper_single_fp<<"bid\t"<<
                                  "bid_coords\t"<<
                                  "tid\t"<<
                                  "gid\t"<<
                                  "strand\t"<<std::endl;
...
int main(int argc, char** argv) {
    ...
    args.add_flag(Opt::STRAND,"strand","consider strand when grouping transcripts",false);
    ...
    globals.consider_strand = args.get_flag(Opt::STRAND);
