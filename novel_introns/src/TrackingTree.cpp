...
void TrackingTree::_convert_gtf(std::string gtf_fname,std::string tissue,std::string base_out_fname){
    std::string gtf_out_fname = base_out_fname+"_"+tissue+".gtf";
...
void TrackingTree::convert(std::string gtf_list_fname,std::string base_out_fname){
    ...
    while (std::getline(gtf_list_stream,aline)) {
        ...
        _convert_gtf(gtf_fname,tissue,base_out_fname);
    }
