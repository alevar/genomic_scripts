...
struct Globals{
    std::ofstream out_seeder_gtf_fp;
    int po_threshold;
} globals;
...
//                }
    ...
    void set_tid(std::string t){
        this->tid = t;
        this->ait.first = this->attrs.find("transcript_id");
        if(this->ait.first!=this->attrs.end()){
            this->ait.first->second = t;
        }
    }

    void increment_tid(){
        int pos = this->tid.find_last_of('.');
        if(pos==std::string::npos){
            this->tid = this->tid+".0";
        }
        else{
            // check if the last substr is int
            int increment;
            std::string increment_str = this->tid.substr(pos+1,this->tid.size()-1);
            std::string base = this->tid.substr(0,pos);
            try {
                increment = std::stoi(increment_str);
                increment+=1;
                this->tid = base+"."+std::to_string(increment);
            }
            catch (...) {
                pos = std::string::npos;
                this->tid = this->tid+".0";
    ...
    static bool le_no_strand(const TX& lhs, const TX& rhs){
        if(lhs.get_seqid()!=rhs.get_seqid()){
            return lhs.get_seqid()<rhs.get_seqid();
        }
        else if(lhs.get_start()!=rhs.get_start()){
            return lhs.get_start()<rhs.get_start();
        }
        else if(lhs.get_end()!=rhs.get_end()){
    ...
    int poverlap(TX& tx,bool consider_strand = true, bool request_contained_in_exon = true){ // computes percent overlap between two transcripts; contained_in_exon if set to true will only compute the number of matching bases if the "this->" transcript is entirely contained within the argument
        if(this->get_seqid()!=tx.get_seqid()){
            return 0;
        }
    ...
    bool is_contained_in_exon(TX& tx){
        for(auto& e : tx.exons){
...
public:
    ...
    bool can_add(TX& t,bool consider_strand=true){
        if(this->size==0){
            return true;
        }
        else{
            if(std::strcmp(this->seqid.c_str(),t.get_seqid().c_str())==0 && (!consider_strand || this->strand==t.get_strand())){
    ...
    // Search for intron-compatible as well as single-exon transcript
    // for single-exon include the following:
    // 1. all reference transcripts contained within the query (multi-exon should be permitted here)
    // 2. all reference transcripts containing query
    // 3. all reference transcripts with >= percent overlap (multi-exon should be permitted)
    //     - multi-exon should be enabled by an optional flag

    // for single-exon comparisons - should not be concerned with the strand

    // For the attributes include the following:
    // 1. reference tid
    // 2. query tid
    // 3. query start and end (since the output should probably contain the reference transcripts)

    int process(Globals& globals){
        // search for intron-compatible transcripts and label accordingly
        std::string t_tid,t_gid,t_name,coord_string;
        for(auto& t : this->txs){
            if(t.is_template()){
                bool found = false;
                for(auto& q : this->txs){
                    if(!q.is_template()){
                        if(t.exon_count()>1 && q.exon_count()>1){
                            if(t.intron_eq(q)){
                                t.add_attribute("seeder_match","1");
                                t.add_attribute("seeder_type","nn");
                                t.add_attribute("seeder_template_tid",t.get_tid());
                                coord_string = q.get_seqid()+q.get_strand()+":"+std::to_string(q.get_start())+"-"+std::to_string(q.get_end());
                                t.add_attribute("seeder_coords",coord_string);
                                t.set_tid(q.get_tid());
                                t.add_attribute("seeder_strand",std::to_string(t.get_strand()!=q.get_strand()));
                                found = true;
                                break;
                            }
                        }
                        else if(t.exon_count()==1){ // evaluate for single-exon transcripts
                            if(q.exon_count()==1){ // both are single-exon
                                if(t.poverlap(q,false,false)>=globals.po_threshold){
                                    t.add_attribute("seeder_match","1");
                                    t.add_attribute("seeder_type","11");
                                    t.add_attribute("seeder_template_tid",t.get_tid());
                                    coord_string = q.get_seqid()+q.get_strand()+":"+std::to_string(q.get_start())+"-"+std::to_string(q.get_end());
                                    t.add_attribute("seeder_coords",coord_string);
                                    t.set_tid(q.get_tid());
                            ...
                            else{ // multi-exon query - check if >= poverlap of template (which is single-exon) is covered without introns by query without interrupting introns
                                if(t.poverlap(q,true,true)>=globals.po_threshold){ // considering strand here because we want to rely on the strandedness of the template and since the query has splicing - the strand should have been resolved correctly here (some rare exceptions might occur in cases of antisense transcripts)
                                    t.add_attribute("seeder_match","1");
                                    t.add_attribute("seeder_type","1n");
                                    t.add_attribute("seeder_template_tid",t.get_tid());
                                    coord_string = q.get_seqid()+q.get_strand()+":"+std::to_string(q.get_start())+"-"+std::to_string(q.get_end());
                                    t.add_attribute("seeder_coords",coord_string);
                                    t.set_tid(q.get_tid());
                ...
                if(found){
                    globals.out_seeder_gtf_fp<<t.get_gtf()<<std::endl; // TODO: do not need all the attributes from the template
                }
                else{
                    t.add_attribute("seeder_match","0");
                    if(t.exon_count()>1){
                        t.add_attribute("seeder_type","n");
                    }
                    else{
                        t.add_attribute("seeder_type","1");
                    }
                    coord_string = t.get_seqid()+t.get_strand()+":"+std::to_string(t.get_start())+"-"+std::to_string(t.get_end());
                    t.add_attribute("seeder_coords",coord_string);
                    globals.out_seeder_gtf_fp<<t.get_gtf()<<std::endl;
                }
            }
        }
        return 0;
...
public:
    ...
    void add(const std::string& gtf_fname,std::string name = "", bool is_templ=false){
        ...
        for(int i=0;i<gffReader.gflst.Count();++i) {
            ...
            if(!name.empty()){
                tmp.add_attribute("seeder_name",name);
            }
            tx_vec.push_back(tmp);
        }
    }

    void sort(bool consider_strand=true){
        if(consider_strand) {
            std::sort(this->tx_vec.begin(), this->tx_vec.end());
        }
        else{
            std::sort(this->tx_vec.begin( ), this->tx_vec.end( ), TX::le_no_strand);
        }
    }
...
// TODO: correct strand of single-exon novel transcripts
//    novel single-exon transcripts may appear on either strand due to the lack of spliced reads.
//    However they should be consistent with the structure of the locus.
//    If the locus contains known isoforms and no antisense transcripts - the novel transcripts should all be on the same strand as the known gene
//    If no known transcripts are found overlapping the novel isoforms - all novel isofors should be on the same strand - can be decided by majority vote


// TODO: should we remove novel loci with too few transcripts? or transcripts that are only single-exon?

int run(std::vector<std::string>& known_gtf_fnames, const std::string& novel_gtf_fname,const std::string& out_fname){
    globals.out_seeder_gtf_fp.open(out_fname);
    ...
    transcriptome.sort(false);

    Bundle bundle;

    for(auto& t : transcriptome){
        if(!bundle.can_add(t,false)){
